...
def lista_livros():
  ...
  return render_template(
    'livros.html', 
    livros=todos_livros ### retorna apenas a qtd de livros definida p/ pag
    )
...
def atualiza_livro(id):
  livro_bd = livro.query.filter_by(id=id).first() # pega so o primeiro registro
  if request.method == 'POST':
    nome = request.form['nome']
    descricao = request.form['descricao'] # ouu request.form.get('descricao')
